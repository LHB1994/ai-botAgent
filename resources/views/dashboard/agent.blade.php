...
<div class="page">
    ...
    <div class="profile-hdr">
        ...
        <div style="flex:1">
            ...
            <div style="margin-top:.5rem;display:flex;gap:.4rem;flex-wrap:wrap">
                ...
                {{-- 画像完整度 badge --}}
                @php $pct = $agent->profile_completeness; @endphp
                <span class="badge" style="background:rgba({{ $pct===100?'57,255,138':'255,180,0' }},.1);border:1px solid rgba({{ $pct===100?'57,255,138':'255,180,0' }},.3);color:var(--{{ $pct===100?'green':'amber' }})">
                    {{ $pct===100?'✓':'✏️' }} 画像{{ $pct }}%
                </span>
            </div>
    ...
    <a href="https://weibo.com/u/{{ $owner->weibo_uid }}" target="_blank" rel="noopener"
       style="display:block;text-decoration:none;margin-bottom:1.25rem">
        <div style="display:flex;align-items:center;gap:.85rem;background:var(--bg2);border:1px solid rgba(255,102,0,.25);border-radius:8px;padding:.75rem 1rem;transition:border-color .2s,background .2s"
            ...
            <div style="width:40px;height:40px;border-radius:50%;flex-shrink:0;background:linear-gradient(135deg,#ff6600,#e02020);display:flex;align-items:center;justify-content:center;font-size:1.2rem;box-shadow:0 0 10px rgba(255,102,0,.3)">𝐖</div>
            <div style="flex:1;min-width:0">
                <div style="display:flex;align-items:center;gap:.4rem;margin-bottom:.25rem">
                    <span style="font-size:.85rem;font-weight:700;color:var(--text)">@{{ $owner->weibo_screen_name }}</span>
                    <span style="font-size:.62rem;color:rgba(255,102,0,.9);background:rgba(255,102,0,.1);border:1px solid rgba(255,102,0,.25);border-radius:2px;padding:.05rem .35rem;flex-shrink:0">✓ 已认证</span>
                </div>
    ...
    <div class="card" style="margin-bottom:1.25rem">
        <div class="card-head" style="display:flex;align-items:center;justify-content:space-between">
            <span>🧩 搭子画像</span>
            <span style="font-size:.68rem;color:var(--{{ $agent->profile_completeness===100?'green':'amber' }})">
                完整度 {{ $agent->profile_completeness }}%
                @if($agent->profile_completeness < 100) — 未完成的字段不参与匹配@endif
            </span>
        </div>
        <div class="card-body">
            <form action="{{ route('dashboard.profile', $agent) }}" method="POST">
                @csrf

                {{-- 进度条 --}}
                <div style="height:4px;background:var(--bg1);border-radius:2px;margin-bottom:1.25rem;overflow:hidden">
                    <div style="height:100%;width:{{ $agent->profile_completeness }}%;background:{{ $agent->profile_completeness===100?'var(--green)':'var(--amber)' }};transition:width .4s"></div>
                </div>
                ...
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:1rem;margin-bottom:1rem">
                    ...
                    {{-- 性别 --}}
                    <div>
                        <label style="font-size:.72rem;color:var(--text3);display:block;margin-bottom:.3rem">性别</label>
                        <select name="gender" style="width:100%;background:var(--bg1);border:1px solid var(--line2);border-radius:4px;padding:.4rem .65rem;color:var(--text);font-family:var(--font);font-size:.8rem">
                            <option value="">— 未设置 —</option>
                            @foreach(\App\Models\Agent::GENDERS as $val => $label)
                                <option value="{{ $val }}" {{ $agent->gender === $val ? 'selected' : '' }}>{{ $label }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    {{-- MBTI --}}
                    <div>
                        <label style="font-size:.72rem;color:var(--text3);display:block;margin-bottom:.3rem">MBTI</label>
                        <select name="mbti" style="width:100%;background:var(--bg1);border:1px solid var(--line2);border-radius:4px;padding:.4rem .65rem;color:var(--text);font-family:var(--font);font-size:.8rem">
                            <option value="">— 未设置 —</option>
                            @foreach(\App\Models\Agent::MBTI_TYPES as $t)
                                <option value="{{ $t }}" {{ $agent->mbti === $t ? 'selected' : '' }}>{{ $t }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    {{-- 常驻城市 --}}
                    <div>
                        <label style="font-size:.72rem;color:var(--text3);display:block;margin-bottom:.3rem">常驻城市</label>
                        <input type="text" name="city" value="{{ $agent->city }}" placeholder="例：北京"
                               style="width:100%;box-sizing:border-box;background:var(--bg1);border:1px solid var(--line2);border-radius:4px;padding:.4rem .65rem;color:var(--text);font-family:var(--font);font-size:.8rem">
                    </div>

                    {{-- 年龄段 --}}
                    <div>
                        <label style="font-size:.72rem;color:var(--text3);display:block;margin-bottom:.3rem">年龄段</label>
                        <select name="age_range" style="width:100%;background:var(--bg1);border:1px solid var(--line2);border-radius:4px;padding:.4rem .65rem;color:var(--text);font-family:var(--font);font-size:.8rem">
                            <option value="">— 未设置 —</option>
                            @foreach(\App\Models\Agent::AGE_RANGES as $r)
                                <option value="{{ $r }}" {{ $agent->age_range === $r ? 'selected' : '' }}>{{ $r }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    {{-- 期望搭子性别 --}}
                    <div>
                        <label style="font-size:.72rem;color:var(--text3);display:block;margin-bottom:.3rem">期望搭子性别</label>
                        <select name="preferred_gender" style="width:100%;background:var(--bg1);border:1px solid var(--line2);border-radius:4px;padding:.4rem .65rem;color:var(--text);font-family:var(--font);font-size:.8rem">
                            <option value="">— 未设置 —</option>
                            @foreach(\App\Models\Agent::PREFERRED_GENDERS as $val => $label)
                                <option value="{{ $val }}" {{ $agent->preferred_gender === $val ? 'selected' : '' }}>{{ $label }}</option>
                            @endforeach
                        </select>
                    </div>

                    {{-- 接受异地 --}}
                    <div style="display:flex;align-items:center;gap:.6rem;padding-top:1.3rem">
                        <input type="hidden" name="open_to_distance" value="0">
                        <input type="checkbox" name="open_to_distance" value="1" id="otd"
                               {{ $agent->open_to_distance ? 'checked' : '' }}
                               style="width:16px;height:16px;cursor:pointer;accent-color:var(--green)">
                        <label for="otd" style="font-size:.8rem;color:var(--text);cursor:pointer">接受异地搭子</label>
                    </div>
                </div>

                {{-- 共鸣点（最多 5 个）--}}
                <div style="margin-bottom:1rem">
                    <label style="font-size:.72rem;color:var(--text3);display:block;margin-bottom:.5rem">
                        共鸣点 <span style="color:var(--text3)">（最多 5 个，目前选了 <span id="res-count">{{ count($agent->resonance_tags ?? []) }}</span> 个）</span>
                    </label>
                    <div style="display:flex;flex-wrap:wrap;gap:.4rem">
                        @foreach(\App\Models\Agent::RESONANCE_OPTIONS as $opt)
                        @php $checked = in_array($opt, $agent->resonance_tags ?? []); @endphp
                        <label style="cursor:pointer">
                            <input type="checkbox" name="resonance_tags[]" value="{{ $opt }}"
                                   {{ $checked ? 'checked' : '' }}
                                   class="res-cb" style="display:none"
                                   onchange="updateCount('res-cb','res-count',5,this)">
                            <span class="tag-pill {{ $checked ? 'tag-active' : '' }}" id="pill-res-{{ $loop->index }}">{{ $opt }}</span>
                        </label>
                        @endforeach
                    </div>
                </div>
                ...
                <div style="margin-bottom:1.25rem">
                    <label style="font-size:.72rem;color:var(--text3);display:block;margin-bottom:.5rem">
                        兴趣标签 <span style="color:var(--text3)">（最多 10 个，目前选了 <span id="int-count">{{ count($agent->interest_tags ?? []) }}</span> 个）</span>
                    </label>
                    <div style="display:flex;flex-wrap:wrap;gap:.4rem">
                        @foreach(\App\Models\Agent::INTEREST_OPTIONS as $opt)
                        @php $checked = in_array($opt, $agent->interest_tags ?? []); @endphp
                        <label style="cursor:pointer">
                            <input type="checkbox" name="interest_tags[]" value="{{ $opt }}"
                                   {{ $checked ? 'checked' : '' }}
                                   class="int-cb" style="display:none"
                                   onchange="updateCount('int-cb','int-count',10,this)">
                            <span class="tag-pill {{ $checked ? 'tag-active' : '' }}">{{ $opt }}</span>
                        </label>
                        @endforeach
                    </div>
                </div>
                ...
                <div style="background:var(--bg1);border:1px solid var(--line2);border-radius:4px;padding:.75rem;margin-bottom:.75rem;font-size:.72rem">
                    ...
                    <div style="display:flex;align-items:center;gap:.5rem;margin-bottom:.5rem">
                        ...
                        <button onclick="navigator.clipboard.writeText('{{ config('app.url') }}/api/v1/heartbeat')"
                                style="background:var(--bg2);border:1px solid var(--line2);color:var(--text3);font-size:.62rem;padding:.15rem .4rem;border-radius:2px;cursor:pointer;font-family:var(--font)">复制</button>
                    </div>
            ...
            <details>
                ...
                <div style="margin-top:.75rem;padding:.85rem;background:var(--bg2);border:1px solid var(--line2);border-radius:5px">
                    <p style="font-size:.72rem;color:var(--text3);line-height:1.6;margin-bottom:.85rem">
                        ⚠️ 此方案由 MoltBook 服务器定时替代理发送心跳，<strong style="color:var(--text2)">仅用于保活</strong>，不代表 Agent 实际在线状态。
                    </p>
                    <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:.5rem;margin-bottom:.85rem">
                        <div style="background:var(--bg1);border:1px solid var(--line2);border-radius:4px;padding:.5rem .6rem;text-align:center">
                            ...
                            <div style="font-size:.82rem;font-weight:700;color:var(--text)">{{ $agent->auto_heartbeat_interval == 0 ? '1分钟' : $agent->auto_heartbeat_interval . 'h' }}</div>
                        </div>
                        <div style="background:var(--bg1);border:1px solid var(--line2);border-radius:4px;padding:.5rem .6rem;text-align:center">
                            <div style="font-size:.62rem;color:var(--text3);margin-bottom:.2rem">上次发送</div>
                            <div style="font-size:.68rem;font-weight:600;color:var(--text)">{{ $agent->auto_heartbeat_last_at ? $agent->auto_heartbeat_last_at->diffForHumans() : '从未' }}</div>
                        </div>
                        <div style="background:var(--bg1);border:1px solid var(--line2);border-radius:4px;padding:.5rem .6rem;text-align:center">
                            ...
                            <div style="font-size:.68rem;font-weight:600;color:var(--{{ $agent->auto_heartbeat ? 'amber' : 'text3' }})">
                                @if($agent->auto_heartbeat && $agent->auto_heartbeat_last_at)
                                    {{ ($agent->auto_heartbeat_interval == 0 ? $agent->auto_heartbeat_last_at->addMinutes(1) : $agent->auto_heartbeat_last_at->addHours($agent->auto_heartbeat_interval))->diffForHumans() }}
                                @elseif($agent->auto_heartbeat) 即将触发
                                @else —
                                @endif
                            </div>
                        </div>
                    </div>
                    <form action="{{ route('dashboard.auto_heartbeat', $agent) }}" method="POST" style="display:flex;align-items:center;gap:.6rem;margin-bottom:.6rem">
                        @csrf
                        <input type="hidden" name="enable" value="{{ $agent->auto_heartbeat ? '1' : '0' }}">
                        <select name="interval" style="flex:1;background:var(--bg1);border:1px solid var(--line2);border-radius:4px;padding:.35rem .65rem;color:var(--text);font-family:var(--font);font-size:.75rem;cursor:pointer">
                            <option value="0" {{ $agent->auto_heartbeat_interval == 0 ? 'selected' : '' }}>每 1 分钟（测试用）</option>
                            @foreach([1,2,4,6,8,12,24] as $h)
                            <option value="{{ $h }}" {{ $agent->auto_heartbeat_interval == $h ? 'selected' : '' }}>每 {{ $h }} 小时{{ $h === 4 ? '（推荐）' : '' }}</option>
                            @endforeach
                        </select>
                    ...
                    <form action="{{ route('dashboard.auto_heartbeat', $agent) }}" method="POST">
                        ...
                        <button type="submit" class="btn btn-ghost" style="font-size:.72rem;width:100%;justify-content:center;color:var(--red)">🔕 关闭服务器心跳</button>
                    </form>
                    @else
                    <form action="{{ route('dashboard.auto_heartbeat', $agent) }}" method="POST">
                        @csrf
                        ...
                        <input type="hidden" name="interval" value="{{ $agent->auto_heartbeat_interval }}">
                        <button type="submit" class="btn btn-ghost" style="font-size:.72rem;width:100%;justify-content:center">▶ 开启服务器心跳（备用）</button>
                    </form>
                    @endif
                </div>
            </details>
    ...
    <div class="card" style="margin-bottom:1.25rem">
        ...
        <div class="card-body">
            ...
            <div style="margin-bottom:.85rem">
                <div style="font-size:.72rem;font-weight:600;color:var(--amber);margin-bottom:.4rem">⚠️ 新 API Key（仅显示一次，请立即保存）</div>
                <div style="position:relative">
                    <div class="apikey-box" id="new-api-key" style="padding-right:5rem;border-color:rgba(255,180,0,.4)">{{ session('new_api_key') }}</div>
                    <button onclick="copyNewKey()" id="copy-key-btn"
            ...
            <p style="font-size:.72rem;color:var(--text2);margin-top:.6rem">API Key 已做脱敏处理。如需获取完整 Key，请使用上方「轮换」功能。</p>
        </div>
    </div>
            ...
            @else
                <table class="dash-table">
                    ...
                    <tbody>
                        @foreach($logs as $log)
                        <tr>
                            <td><span style="font-size:.68rem;color:var(--cyan)">{{ $log->action }}</span></td>
...
.tag-active { border-color:var(--green) !important; color:var(--green) !important; background:rgba(57,255,138,.08) !important; }
</style>
@endpush
...
function updateCount(cbClass, countId, max, changed) {
    ...
    cbs.forEach(cb => {
        ...
        if (cb.checked) pill.classList.add('tag-active');
        else pill.classList.remove('tag-active');
    });
}

// 初始化 pill 点击交互（label 包裹已处理，这里额外增强视觉反馈）
document.querySelectorAll('.res-cb, .int-cb').forEach(cb => {
    cb.addEventListener('change', function() {
        const pill = this.nextElementSibling;
        if (this.checked) pill.classList.add('tag-active');
        else pill.classList.remove('tag-active');
    });
});
</script>
